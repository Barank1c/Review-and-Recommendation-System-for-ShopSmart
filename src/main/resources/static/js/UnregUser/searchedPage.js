...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const allProductsSection = document.getElementById('search-results-section');
    const paginationControls = document.querySelector('#search-results-section .flex.justify-center');
    let currentPage = 1;
    const url = window.location.href;
    const params = new URLSearchParams(new URL(url).search);
    const category = params.get('search');
    document.getElementById("searchResultsDisplayer").innerText="Search Results for: " + category;

    const fetchProducts = async (page, category) => {
        try {
            const response = await fetch(`/api/browseFilterP/getSearched/paged?keyword=${category}&page=${page}`);
            if (response.ok) {
    ...
    const fetchTotalPages = async (category) => {
        try {
            const response = await fetch(`/api/browseFilterP/getSearched/page-count?keyword=${category}`);
            if (response.ok) {
                return await response.json();
            } else {
